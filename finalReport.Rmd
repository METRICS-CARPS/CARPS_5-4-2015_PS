...
#here there is a mis-match for the mean- this might be the condition with the excluded participant.
...
reportObject <- reproCheck(reportedValue = "<.05", obtainedValue = .01902, valueType = 'pes', eyeballCheck = TRUE) #p value
```

## Step 5: Conclusion

We encountered some discrepencies between the reported findings and our own obtained values. We found major errors in F statistic and effect size statistic (partial eta squared) for the ANOVA. While the authors noted in the text that one participant was removed from analyses due to a computer error, it was not clear from the data file which participant was removed. This amounts to an insufficient information error. We suspect that this lead to the major errors in F and PES, a minor error in the mean for one condition (time poor - large account), and a minor error in degrees of freedom in the ANOVA. There were also minor errors in the confidence intervals around the condition means, which could possibly be due to differences in the calculation of CIs across packages.
